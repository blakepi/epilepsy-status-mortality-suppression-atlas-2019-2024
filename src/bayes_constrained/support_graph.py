...
def analyze_state_support(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for state_fips, state_frame in frame.groupby("state_fips", sort=True):
        ...
        four_cycle_keys = {
            key for key, pair in edge_lookup.items() if pair in four_cycle_pairs
        }
        ...
        for component in components:
            component_edges = {
                key for key in edge_lookup if all(node in component for node in key)
            }
    ...
    payload.update(
        {
            ...
            "states_with_cyclic_component_without_four_cycle": int((state_summary["cyclic_components_without_four_cycle"] > 0).sum()),
        }
    )
    return payload
