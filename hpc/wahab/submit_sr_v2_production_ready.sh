...
python scripts/84_validate_sr_v2_final_production_infrastructure.py
python scripts/82_prepare_sr_v2_production_final.py
bash hpc/wahab/stage_sr_v2_to_scratch_final.sh
...
finalize_job="$(sbatch --parsable --dependency="afterok:${production_job}" hpc/wahab/slurm/62_sr_v2_finalize_production_final.sbatch)"
...
printf '%s\tfinalize\t%s\tafterok:%s\t%s\n' \
  "$submitted_at" "$finalize_job" "$production_job" "hpc/wahab/slurm/62_sr_v2_finalize_production_final.sbatch" >> "$record"
