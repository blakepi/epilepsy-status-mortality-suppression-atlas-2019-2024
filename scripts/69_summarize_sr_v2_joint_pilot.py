...
def proposal_multiplier_recommendations(acceptance: pd.DataFrame, current: dict[str, float]) -> dict[str, float]:
...
def main() -> None:
    ...
    if len(chain_dirs) != 4:
        raise SystemExit(f"Expected four joint-pilot chain directories; found {len(chain_dirs)}.")
    ...
    for chain_dir in chain_dirs:
        ...
        for draw_index in range(latent.shape[0]):
            assert_constraints(latent[draw_index], frame, label=f"joint_pilot_chain{chain}_draw{draw_index + 1}")
    ...
    parameter_diagnostics.to_csv(output_root / "joint_pilot_parameter_diagnostics.csv", index=False)
    acceptance.to_csv(output_root / "joint_pilot_acceptance.csv", index=False)
    status_df.to_csv(output_root / "joint_pilot_chain_status.csv", index=False)

    primary_rows: list[dict[str, object]] = []
    archived = pd.read_csv(ROOT / "outputs" / "bayes_constrained" / "production_8chain" / "posterior_parameter_summary.csv").set_index("parameter")
    for parameter in PRIMARY_PARAMETERS:
        ...
        diag = parameter_diagnostics.set_index("parameter").loc[parameter]
        primary_rows.append(
            {
                "parameter": parameter,
    ...
    primary.to_csv(output_root / "joint_pilot_primary_estimates.csv", index=False)
    ...
    for column, prefix in [("primary_rurality", "rurality"), ("svi_quartile", "svi")]:
        for category in sorted(frame[column].astype(str).unique()):
            category_masks[f"latent_{prefix}_total[{category}]"] = frame[column].astype(str).eq(category).to_numpy()
    ...
    movement.to_csv(output_root / "joint_pilot_latent_movement.csv", index=False)
    ...
    latent_diagnostics.to_csv(output_root / "joint_pilot_latent_summary_diagnostics.csv", index=False)

    resolved_config = yaml.safe_load((output_root / "joint_pilot_chain_01_config.yaml").read_text(encoding="utf-8"))
    current_multipliers = {
        key: float(value)
    ...
    acceptance_summary.to_csv(output_root / "joint_pilot_acceptance_summary.csv", index=False)

    primary_diag = parameter_diagnostics[parameter_diagnostics["parameter"].isin(PRIMARY_PARAMETERS)]
    summary = {
        "chains_completed": int(status_df["status"].eq("completed").sum()),
        "draws_per_chain": int(min(array.shape[0] for array in latent_arrays.values())),
        ...
        "median_final_fraction_free_cells_changed": float(
            movement.sort_values("draw").groupby("chain", as_index=False).tail(1)["fraction_free_cells_changed"].median()
        ),
        ...
        "pilot_pass": bool(
            ...
            and float(primary_diag["r_hat"].max()) < 1.25
        ),
        "interpretation_boundary": "Short tuning pilot only; no corrected epidemiologic estimate is authorized.",
    }
    (output_root / "joint_pilot_summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    (output_root / "recommended_joint_tuning.yaml").write_text(
        yaml.safe_dump(
            {
                ...
                "source": "Scientific Reports v2 four-chain short joint pilot",
    ...
    lines = [
        ...
        f"- Retained draws per chain: {summary['draws_per_chain']}",
        ...
        f"- Maximum rurality/SVI latent-summary R-hat: {summary['maximum_latent_summary_rhat']:.4f}",
        f"- Median final fraction of free cells changed: {summary['median_final_fraction_free_cells_changed']:.4f}",
        f"- Pilot status: {'PASS' if summary['pilot_pass'] else 'HOLD'}",
        "",
    ...
    for _, row in primary.iterrows():
        lines.append(
            ...
            f"{row['pilot_relative_shift_percent']:.2f}% | {row['r_hat']:.4f} |"
        )
    lines.extend(
        [
            "",
    ...
    (output_root / "joint_pilot_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    if not summary["pilot_pass"]:
        raise SystemExit("Corrected joint pilot did not pass its gross-failure gate.")
