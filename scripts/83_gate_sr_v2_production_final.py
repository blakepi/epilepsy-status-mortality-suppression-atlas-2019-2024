...
def main() -> None:
    if not CONFIG_PATH.exists():
        raise SystemExit(f"Missing frozen v2 production config: {CONFIG_PATH}")
    ...
    parameter_finite = np.isfinite(
        parameters[["r_hat", "ess_bulk", "ess_tail", "mcse_mean"]].to_numpy(
            dtype=float
        )
    ).all()
    ...
    unique_values = (
        latent_draws.groupby("parameter")["value"]
        .nunique()
        .rename("unique_values")
        .reset_index()
    )
    latent_eval = latent.merge(unique_values, on="parameter", how="left")
    ...
    add(
        "latent_summary_rhat",
        latent_finite
        and float(stochastic["r_hat"].max())
        <= float(thresholds["latent_summary_rhat_max"]),
        (
    ...
    add(
        ...
        latent_finite
        and float(stochastic["ess_bulk"].min())
        >= float(thresholds["latent_summary_ess_bulk_min"]),
        (
    ...
    add(
        ...
        latent_finite
        and float(stochastic["ess_tail"].min())
        >= float(thresholds["latent_summary_ess_tail_min"]),
        (
    ...
    add(
        "extended_real_pilot_gate",
        bool(extended_real.get("pilot_pass")),
        str(extended_real.get("pilot_pass")),
    )
    add(
    ...
    add(
        "tuned_calibration_extension_gate",
        bool(calibration_extension.get("computational_gate_pass")),
        str(calibration_extension.get("computational_gate_pass")),
    )
    ...
    result = {
        ...
        "worst_stochastic_latent_summary": (
            diagnostic_record(worst_latent) if worst_latent is not None else None
        ),
        "interpretation_boundary": (
            "Passing this gate freezes corrected computational results. Scientific Reports "
            "submission still requires multi-replicate calibration, prespecified epidemiologic "
            "robustness analyses, and final manuscript-package QC."
        ),
    }
    ...
    if not passed:
        raise SystemExit(
            "Scientific Reports v2 final production gate is on HOLD; evidence was written."
        )
