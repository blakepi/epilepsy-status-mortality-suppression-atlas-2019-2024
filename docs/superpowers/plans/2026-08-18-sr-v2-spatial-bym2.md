...
- **Corrective amendment (2026-08-18):** the spatial execution YAML's initial intercept-prior entry `Normal(mean=0, sd=5)` contradicted the canonical primary code and manuscript. The authoritative prior is `Normal(mean_rule=crude_national_log_rate, sd=5)`, with the mean derived only by `crude_intercept_prior` from the frozen model frame. The target fingerprint binds `intercept_mean_rule`, the exact `intercept_mean_float_hex`, the raw frozen source authority, and `model_frame_semantic_sha256`; save and resume reject any runtime mismatch. This amendment corrects the specification before production and does not change the immutable v1.1.1 primary baseline.
...
Every attempt carries one exact `sr_v2_spatial_resume_from/v1` certificate in both its immutable status and immutable attempt evidence. It binds the source immutable-status path/hash, source checkpoint path/hash, source epoch/attempt/iteration/saved-draw position and, for an extension, the rebound epoch-start checkpoint path/hash. The independent verifier validates every status in custody, requires an attempt to start at its certified saved-draw position, and proves that an extension rebound preserves state, RNG, target density, counters, chunks, pending buffers, adaptation, next draw, and output positions; only reviewed identity/epoch/attempt fields may change. It also independently recomputes `count_moves` from the frozen frame/settings and requires the five latent proposal totals and every-25th blocked-refresh totals at every prepared, resume-boundary, rebound, and terminal checkpoint.
