...
Eight corrected production chains were initialized from independently seeded feasible allocations and run for 300,000 iterations per chain, with 75,000 iterations discarded and every 50th subsequent state retained. Across all monitored parameters the worst rank-normalized split R-hat was 1.006 (state_effect[15]), with bulk effective sample size 2,071 and tail effective sample size 4,229; the worst stochastic latent summary was latent_county_period[48035] at R-hat 1.0015. Every retained latent state satisfied integrality, nonnegativity, county-year bounds, county-period restrictions, state-year equalities, national-year reconciliation, and the grand total. Representative stochastic latent summaries met the prespecified between-chain and effective-sample-size thresholds. Parameter-block and count-move acceptance rates are reported in Supplementary Table {{SUPPLEMENT_ACCEPTANCE_TABLE}}.

### Primary rurality and social-vulnerability associations

Relative to large metropolitan counties, the corrected posterior mortality rate ratios were other metropolitan, 1.14 (95% credible interval, 1.10 to 1.18); nonmetro adjacent, 1.27 (95% credible interval, 1.22 to 1.33); nonmetro nonadjacent, 1.23 (95% credible interval, 1.17 to 1.30). Social vulnerability showed a monotone gradient: quartile 2, 1.11 (95% credible interval, 1.07 to 1.16); quartile 3, 1.25 (95% credible interval, 1.20 to 1.30); the most vulnerable quartile, 1.43 (95% credible interval, 1.37 to 1.50), each relative to the least vulnerable quartile. The primary nonmetro nonadjacent contrast was 1.23 (95% credible interval, 1.17 to 1.30), with posterior probability greater than 0.999 that the rate ratio exceeded 1. Complete posterior summaries, rank-normalized split R-hat, bulk ESS, tail ESS, and Monte Carlo standard errors for all 69 monitored parameters are provided in Supplementary Table {{SUPPLEMENT_PARAMETER_TABLE}}.
...
County-period posterior mortality summaries are ecological model quantities, not observed or recovered suppressed counts. They are reported for all 3,142 counties, each with a posterior mean, median, and 95% credible interval on both the count and the rate scale. Residual spatial dependence was evaluated using the U.S. Census Bureau county-adjacency file and a prespecified permutation Moran statistic applied to posterior-mean county-period residuals. Global Moran's I was 0.117 for the conditional Pearson residual across 3,142 counties (3,128 with at least one model neighbor), with a two-sided permutation P value of 0.0001, the smallest value attainable with 9,999 permutations; 19 within-state Pearson statistics also met the prespecified materiality rule, so the prespecified structured county spatial-effect sensitivity was triggered. If the prespecified residual-spatial threshold was met, a structured county spatial-effect sensitivity model was fitted and compared with the primary state-effect model: {{SPATIAL_MODEL_RESULT}}.
...
17. U.S. Census Bureau. 2024 County Adjacency File. https://www2.census.gov/geo/docs/reference/county_adjacency/county_adjacency2024.txt. Accessed 2026-08-18.

## Locked placeholders

The following placeholders must remain unresolved until their stated gate passes:

- `1.23`, `1.17 to 1.30`, all primary and SVI result fields: corrected eight-chain production gate.
- `Global Moran's I was 0.117 for the conditional Pearson residual across 3,142 counties (3,128 with at least one model neighbor), with a two-sided permutation P value of 0.0001, the smallest value attainable with 9,999 permutations; 19 within-state Pearson statistics also met the prespecified materiality rule, so the prespecified structured county spatial-effect sensitivity was triggered.`: post-production spatial residual diagnostic.
